...
class RobotController:
    def __init__(
        self, kp, ki, kd, wheel_distance, v_max, position_threshold, yaw_threshold
    ):
    ...
    def control(self, x_robot, y_robot, theta_current, waypoints, target_yaws):
        ...
        distance_to_target = math.sqrt(
            (x_target - x_robot) ** 2 + (y_target - y_robot) ** 2
        )
        if (
            distance_to_target < self.position_threshold
            and abs(self.normalize_angle(theta_current - theta_target))
            < self.yaw_threshold
        ):
        ...
        delta_theta = (
            blend_factor * delta_theta_position + (1 - blend_factor) * delta_theta_yaw
        )
...
while waypoints:
    v_left, v_right = controller.control(
        x_robot=0,
        y_robot=0,
        theta_current=0,
        waypoints=waypoints,
