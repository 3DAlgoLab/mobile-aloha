...
def main(args):
    ...
    if use_actuator_net:
        ...
        actuator_network_path = os.path.join(
            actuator_network_dir, "actuator_net_last.ckpt"
        )
        loading_status = actuator_network.load_state_dict(
            torch.load(actuator_network_path)
        )
        actuator_network.eval()
        actuator_network.cuda()
        print(
            f"Loaded actuator network from: {actuator_network_path}, {loading_status}"
        )

        actuator_stats_path = os.path.join(
            actuator_network_dir, "actuator_net_stats.pkl"
        )
        ...
        out_unnorm_fn = (
            lambda x: (x * actuator_stats["commanded_speed_std"])
            + actuator_stats["commanded_speed_mean"]
        )
        ...
        for t in range(0, episode_len, prediction_len):
            ...
            actuator_net_in = (
                torch.from_numpy(actuator_net_in).float().unsqueeze(dim=0).cuda()
            )
