...
start_pose = get_pose()
delta_target_pose_l = [
    np.array([r * np.sin(theta), -(r - r * np.cos(theta)), -theta])
    for theta in np.linspace(0, np.pi / 2, int((np.pi / 2) // (0.01 / r)))
]
target_pose_l = []
for delta_target_pose in delta_target_pose_l:
    transformed_delta_target_pose_x = delta_target_pose[0] * np.cos(
        start_pose[2]
    ) - delta_target_pose[1] * np.sin(start_pose[2])
    transformed_delta_target_pose_y = delta_target_pose[0] * np.sin(
        start_pose[2]
    ) + delta_target_pose[1] * np.cos(start_pose[2])
...
for target_pose in target_pose_l:
    ...
    while True:
        ...
        target_heading = np.arctan2(
            target_pos[1] - curr_pos[1], target_pos[0] - curr_pos[0]
        )
        ...
        if distance_to_target < POS_THRESHOLD:
            ...
            is_forward = np.sign(
                np.dot(yaw_to_vector(target_heading), yaw_to_vector(curr_orn))
            )
        ...
        if args.debug:
            tracer.SetMotionCommand(linear_vel=0, angular_vel=0.1)
            time.sleep(1)
        else:
            # set motion command
            tracer.SetMotionCommand(linear_vel=v, angular_vel=w)
            time.sleep(0.05)
